...
const columns = [
  ...
  },
  {
    ...
    sortable: true,
    textAlign: "right" as any,
    render: (row) => Number(row.profit).toFixed(2),
  },
];
...
export default function ProfitReport() {
  ...
  if (checkSuccess(queries)) {
    const { profit } = calculator(transactions.data);
    return (
      <>
        <ActionIcon
          onClick={() => {
            ...
            <Group justify='end'>
              <Table
                ...
                data={{
                  body: [
                    [
                      <Text key={"profit"} fw={700}>
                        Total Profit
                      </Text>,
                      <Group key={"profit amount"} justify='end'>
                        <Text fw={700}>{profit.toFixed(2)}</Text>
                      </Group>,
                    ],
                  ],
                }}
...
function calculator(data) {
  const profit = data.reduce(
    (total, record) => (record.type === "sale" ? record.profit + total : total - record.profit),
    0
  );
  return { profit };
}
